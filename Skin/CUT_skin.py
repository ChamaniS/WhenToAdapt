...
def train_cut(domainA_dir, domainB_dir, save_dir, epochs=CUT_EPOCHS, device=DEVICE,
    ...
    # Feature channels for our generator layers
    ngf = 64
    layer_channels = {
        1: ngf,
        2: ngf * 2,
    ...
    for epoch in range(epochs):
        ...
        for real_A in loop:
            ...
            real_B = real_B.to(device)  # [0,1]

            inp_A = real_A * 2.0 - 1.0  # [-1,1]
            ...
            for l in nce_layers:
                ...
                # match by same patch index order
...
def harmonize_folder_with_generator(generator, src_img_dir, dst_img_dir,
    ...
    for p in image_files:
        ...
        if mask_src_dir is not None and mask_dst_dir is not None:
            # preserve original mask filename if possible
...
def main():
    ...
    # ------------------------------------------------------------------
    # Step 1: train CUT models for each client -> reference domain
    # ------------------------------------------------------------------
    cut_models = {}
    ...
    # ------------------------------------------------------------------
    # Optional: save sample comparisons
    # ------------------------------------------------------------------
    ...
    # If your UNET uses out_channels instead of num_classes, replace the line above with:
