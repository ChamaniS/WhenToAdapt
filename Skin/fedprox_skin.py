...
# -------------------------
# Training / Eval functions (FedProx)
# -------------------------
def train_local(loader, model, loss_fn, opt, global_state_dict=None, mu=0.0):
    ...
    if use_prox:
        global_params = {k: v.detach().clone().to(DEVICE) for k, v in global_state_dict.items()}
...
def plot_metrics(round_metrics, out_dir):
    ...
    plt.ylabel("Dice")
    plt.title("Per-client Dice (FedProx)")
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(out_dir, "dice_no_bg_fedprox.png"))
    plt.close()
    ...
    plt.xlabel("Global Round")
    plt.ylabel("IoU")
    plt.title("Per-client IoU (FedProx)")
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(out_dir, "iou_no_bg_fedprox.png"))
    plt.close()
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(out_dir, "global_dice_no_bg_fedprox.png"))
    plt.close()
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(out_dir, "global_iou_no_bg_fedprox.png"))
    plt.close()
...
def main():
    ...
    for r in range(COMM_ROUNDS):
        ...
        # snapshot the global model for the FedProx proximal term
        global_state_for_prox = {k: v.detach().clone() for k, v in global_model.state_dict().items()}
        ...
        for i in range(NUM_CLIENTS):
            ...
            train_local(
                ...
                loss_fn,
                opt,
                global_state_dict=global_state_for_prox,
                mu=PROX_MU
            )
        ...
        # FedAvg aggregation after FedProx local training
