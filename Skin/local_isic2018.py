...
# -------------------------
# Dataset that pairs images + masks by stem and accepts .png masks
# -------------------------
class SkinPairDataset(Dataset):
    def __init__(self, img_dir, mask_dir, transform=None, img_exts=None, mask_exts=None):
        ...
        transform: albumentations transform applied with transform(image=..., mask=...)
        img_exts, mask_exts: tuples of allowed extensions
        """
...
def accumulate_binary_counts(preds_label, targets):
    """
    Computes TP/TN/FP/FN on the foreground class (class 1).
    preds_label and targets should contain 0/1 labels.
    """
...
def eval_performance(loader, model, loss_fn, num_classes):
    ...
    epoch_acc = 100.0 * (valid_running_correct / num_batches)
    epoch_iou_class = [v / num_batches for v in valid_iou_score_class]
...
def test(loader, model, loss_fn, num_classes):
    ...
    epoch_iou_class = [v / num_batches for v in test_iou_score_class]
    epoch_iou_withbackground = sum(epoch_iou_class) / num_classes
    epoch_iou_nobackground = sum(epoch_iou_class[1:]) / (num_classes - 1) if num_classes > 1 else 0.0
...
def train(train_loader, model, optimizer, scheduler, loss_fn, num_classes):
    ...
    avg_ious = [v / num_batches for v in iou_classes]
...
def main():
    ...
    TRAIN_IMG_DIR = r"C:\Users\csj5\Projects\Data\skinlesions\ISIC2018\train\images"
    TRAIN_MASK_DIR = r"C:\Users\csj5\Projects\Data\skinlesions\ISIC2018\train\masks"
    VAL_IMG_DIR = r"C:\Users\csj5\Projects\Data\skinlesions\ISIC2018\val\images"
    VAL_MASK_DIR = r"C:\Users\csj5\Projects\Data\skinlesions\ISIC2018\val\masks"
    TEST_IMG_DIR = r"C:\Users\csj5\Projects\Data\skinlesions\ISIC2018\test\images"
    TEST_MASK_DIR = r"C:\Users\csj5\Projects\Data\skinlesions\ISIC2018\test\masks"
    ...
    train_loader = get_loaders_train(
        dataset_class, TRAIN_IMG_DIR, TRAIN_MASK_DIR, BATCH_SIZE,
        train_transform, NUM_WORKERS, img_exts=img_exts, mask_exts=mask_exts
    )
    val_loader = get_loaders_val(
        dataset_class, VAL_IMG_DIR, VAL_MASK_DIR, BATCH_SIZE,
        val_transform, NUM_WORKERS, img_exts=img_exts, mask_exts=mask_exts
    )
    test_loader = get_loader_test(
        dataset_class, TEST_IMG_DIR, TEST_MASK_DIR,
        val_transform, img_exts=img_exts, mask_exts=mask_exts
    )
    ...
    for epoch in range(NUM_EPOCHS):
        print(f"\n[INFO]: Epoch {epoch + 1} of {NUM_EPOCHS}")
        ...
        if val_iou_no_bg > best_iou:
            ...
            torch.save(model.state_dict(), "BestModels/best_model_centralized_Unet_isic2018.pth")
            print("Model saved!")

    print("[INFO]: Testing the best model...")
    model.load_state_dict(torch.load("BestModels/best_model_centralized_Unet_isic2018.pth", map_location=DEVICE))
