...
# MixStyle settings
APPLY_MIXSTYLE = True
MIXSTYLE_P = 1.0   # apply on every training batch
MIXSTYLE_ALPHA = 0.2
...
# -------------------------
def input_mixstyle(images, p=1.0, alpha=0.2, eps=1e-6):
    """
    ...
    # force different partners
    perm = torch.roll(torch.arange(B, device=images.device), shifts=1)

    # stronger visible mixing for visualization
    lam = torch.full((B, 1, 1, 1), 0.5, device=images.device)
...
# -------------------------
def _tensor_to_uint8_img(img_t):
    ...
    for split in ["train", "val", "test"]:
        ...
        for fname in os.listdir(img_dir):
            if fname.lower() == target_lower:
                ...
                # build a temporary dataset just to reuse its extension logic
...
def save_mixstyle_comparison_grid(selected_items, transform, out_base, grid_name="mixstyle_four_dataset_comparison.png"):
    """
    Saves only one comparison grid:
      row 1: original
      row 2: mixstyle output
    ...
    # Apply MixStyle once for the visualization batch
    mixed = input_mixstyle(batch, p=1.0, alpha=MIXSTYLE_ALPHA).clamp(0.0, 1.0)

    originals = batch.detach().cpu()
    mixed_cpu = mixed.detach().cpu()

    diffs = (mixed_cpu - originals).abs()
    diffs = torch.clamp(diffs * 6.0, 0.0, 1.0)  # amplify for visibility
    ...
    for i in range(n):
        orig_img = _tensor_to_uint8_img(originals[i])
        mix_img = _tensor_to_uint8_img(mixed_cpu[i])
        diff_img = _tensor_to_uint8_img(diffs[i])
    ...
    print(f"Saved MixStyle comparison grid to: {out_path}")
...
def plot_metrics(round_metrics, out_dir):
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(out_dir, "dice_no_bg_mixstyle.png"))
    plt.close()
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(out_dir, "iou_no_bg_mixstyle.png"))
    plt.close()
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(out_dir, "global_dice_no_bg_mixstyle.png"))
    plt.close()
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(out_dir, "global_iou_no_bg_mixstyle.png"))
    plt.close()
...
def main():
    ...
    # Save ONLY one comparison grid for the 4 exact images
    selected_items = [
    ...
    save_mixstyle_comparison_grid(
        selected_items=selected_items,
        transform=val_tf,
        out_base=os.path.join(out_dir, "MixStyleSamples"),
        grid_name="mixstyle_four_dataset_comparison.png"
    )
