...
# -------------------------
PFEDME_LAMBDA = 10.0   # proximal regularization strength
INNER_STEPS = 10       # number of inner updates for personalized model theta
INNER_LR = 5e-3        # learning rate for inner optimization
OUTER_ETA = 1e-4       # outer update step for local model w
BETA = 1.0             # server mixing factor (1.0 = full average, <1.0 = partial update)
...
client_ext_map = {
    ...
    "ISIC2017": ((".jpg",), (".png",)),
    "ISIC2018": ((".jpg",), (".png",)),
    # PH2: include common possibilities; adjust if you know exact ones
...
def compute_metrics(pred, target, smooth=1e-6):
    """
    Binary segmentation metrics from logits pred and binary target.
    Returns averaged batch metrics.
    """
...
def average_models_weighted(models, weights):
    """
    Weighted average of all model parameters, including BN parameters.
...
def train_local_pfedme(
    ...
    """
    Local pFedMe-style optimization:
    - model = local/server-facing personalized model w
    - theta = inner personalized model updated on local batch
    - proximal term keeps theta close to w
    """
    model.train()
    total_loss = 0.0
    metrics = []
    ...
    for _epoch in range(LOCAL_EPOCHS):
        for data, target in tqdm(loader, leave=False):
            ...
            # Inner loop: solve for personalized theta
            ...
            # Outer update on w (move toward theta)
            with torch.no_grad():
                for p_w, p_theta in zip(model.parameters(), theta.parameters()):
                    p_w.data = p_w.data - (outer_eta * lam) * (p_w.data - p_theta.data)

            # Optional: report training loss/metrics on the updated local model
            preds = model(data)
...
def compute_personalized_model_from_w(
    ...
    """
    Given a client model w, compute personalized theta by running inner pFedMe steps.
    """
...
def main():
    ...
    for r in range(COMM_ROUNDS):
        ...
        for i in range(NUM_CLIENTS):
            # Start each client from current global model
        ...
        # Evaluate personalized models on each client's test set
        rm = {}
        for i in range(NUM_CLIENTS):
        ...
        # Global test on union of all client test sets
