# fl_train_fedper.py
"""
Federated training with FedPer for TB CXR classification.

Behavior:
 - Shares the lower layers (feature extractor) across clients (FedPer aggregation).
 - Keeps the classification head (task-specific last layer) local to each client.

Usage:
 - Update CLIENT_ROOTS and CONFIG at top as before, then:
     python fl_train_fedper.py
"""
import os
...
# ----------------------------
# --- FedPer helper utilities
# ----------------------------
# Patterns to detect classification head keys (lowercase matching)
HEAD_KEY_PATTERNS = (
    "classifier.weight", "classifier.bias",
    "fc.weight", "fc.bias",
    "head.weight", "head.bias",
    "head.fc.weight", "head.fc.bias",
    # sometimes timm uses "classifier.1.weight" (Sequential) etc. we'll check suffixes
)

def is_head_key(key: str) -> bool:
    kl = key.lower()
    for p in HEAD_KEY_PATTERNS:
        if kl.endswith(p):
            return True
    # also treat keys that start with typical classifier modules
    # e.g., "...classifier.1.weight", "...classifier.1.bias"
    if ".classifier." in kl or ".fc." in kl or ".head." in kl:
        # but avoid batchnorm or conv layers that may contain "head" incorrectly:
        # we'll still accept these as head keys only if they are last layers (heuristic)
        parts = kl.split(".")
        # if last two tokens include weight/bias -> consider head
        if len(parts) >= 2 and parts[-1] in ("weight","bias"):
            return True
    return False

def get_shared_keys_from_state_dict(sd: dict) -> List[str]:
    # Shared keys are all keys except those that correspond to the head/classifier
    shared = [k for k in sd.keys() if not is_head_key(k)]
    return shared

def fedper_aggregate_shared_params(local_models: List[torch.nn.Module], weights: List[float], device="cpu"):
    """
    Average only shared parameters across local_models according to weights (FedPer).
    local_models are assumed to be CPU (or same device) models (state_dict accessible).
    Returns a dict mapping shared_key -> averaged tensor (on device).
    """
    if len(local_models) == 0:
        return {}
    # collect state dicts (as tensors on CPU)
    sds = [lm.state_dict() for lm in local_models]
    # determine shared keys using first model
    all_keys = list(sds[0].keys())
    shared_keys = get_shared_keys_from_state_dict(sds[0])
    # average each shared key
    avg_shared = {}
    for k in shared_keys:
        # accumulate weighted tensors on CPU
        accum = None
        for i, sd in enumerate(sds):
            v = sd[k].cpu().clone().detach()
            w = float(weights[i])
            if accum is None:
                accum = w * v
            else:
                accum += w * v
        avg_shared[k] = accum.to(device)
    return avg_shared

def update_state_dict_with_shared(sd: dict, shared_updates: dict):
    """Update state dict sd (mapping) in-place for keys present in shared_updates."""
    for k, v in shared_updates.items():
        sd[k] = v.clone() if isinstance(v, torch.Tensor) else torch.tensor(v, device=sd[next(iter(sd))].device)
    return sd
...
# ----------------------------
# Federated training main (with FedPer)
# ----------------------------
def main():
    print("DEVICE:", DEVICE)
    ...
    for r in range(COMM_ROUNDS):
        ...
        for i, client in enumerate(per_client_dataloaders):
            ...
            # move model to CPU for safe aggregation (and to save GPU mem)
        ...
        # --------------------------
        # FedPer aggregation: average only shared (feature extractor) params
        # --------------------------
        print("\nAggregating local models (FedPer shared-params weighted average)")
        # compute averaged shared params (on CPU then move to DEVICE)
        avg_shared = fedper_aggregate_shared_params(local_models, weights, device=DEVICE)

        # Update global model's shared params only
        global_sd = global_model.state_dict()
        # update keys present in avg_shared
        for k, v in avg_shared.items():
            if k in global_sd:
                global_sd[k] = v.clone().to(global_sd[k].device)
        global_model.load_state_dict(global_sd)
        global_model.to(DEVICE)

        # Sync shared params from updated global model back to each local model (so personalization heads kept local)
        for lm in local_models:
            lm_sd = lm.state_dict()
            for k, v in avg_shared.items():
                if k in lm_sd:
                    lm_sd[k] = v.clone().cpu()
            lm.load_state_dict(lm_sd)
        # Note: local_models remain CPU copies used only for aggregation; the actual 'global_model' on DEVICE will be used below for evaluation.
        ...
        # Global test per client (evaluate the personalized local models)
        per_client_test_metrics = []
        for i, client in enumerate(per_client_dataloaders):
            print(f"\nGlobal TEST on client {i} ({CLIENT_NAMES[i]}) test set")
            # we need to evaluate the personalized model that client would have:
            # reconstruct client's personalized model: start from global_model, then load that client's local head
            # We have local_models list as CPU models whose shared params were synced but whose heads remained local.
            # Find corresponding local model (CPU) and move to DEVICE for evaluation.
            local_model_cpu = local_models[i]
            # move to device
            local_model = copy.deepcopy(local_model_cpu).to(DEVICE)
