...
This variant integrates FedBN: BatchNorm parameters & running stats are kept local
and NOT aggregated; all other parameters are aggregated weighted by client size.
...
WORKERS = 4
LOCAL_EPOCHS = 1      # local epochs per communication round (as requested)
...
# ----------------------------
# FedBN-aware aggregation
# ----------------------------
def fedbn_average_models(models: List[torch.nn.Module], weights: List[float], global_state_dict: dict):
    """
    FedBN aggregation:
      - Average all parameters EXCEPT BatchNorm parameters and running stats.
      - For BN params / stats (keys containing 'bn' or 'running_mean'/'running_var'/'num_batches_tracked'),
        we keep the values from the current global_state_dict (i.e., do NOT override client's local BN).
    Assumes models[...] are CPU or compatible tensors.
    """
    # start from a copy of first model's state dict to keep shapes / keys
    avg_sd = copy.deepcopy(models[0].state_dict())

    # Helper to identify BN keys (affine params and running stats)
    def is_bn_key(key: str):
        k = key.lower()
        # common BN related substrings: 'bn', 'running_mean', 'running_var', 'num_batches_tracked'
        if ('bn' in k) or ('running_mean' in k) or ('running_var' in k) or ('num_batches_tracked' in k):
            return True
        # some architectures have names like 'downsample.1' referencing BN - include that
    ...
    for key in avg_sd.keys():
        if is_bn_key(key):
            # keep global BN param/stat (do not average) -> get from provided global_state_dict
            if key in global_state_dict:
                avg_sd[key] = global_state_dict[key].cpu()
            else:
                # fallback: keep first model's BN
                avg_sd[key] = models[0].state_dict()[key].cpu()
            continue
        # otherwise average across clients (weighted)
        # ensure tensors are on CPU to avoid device issues
        avg_sd[key] = sum(weights[i] * models[i].state_dict()[key].cpu() for i in range(len(models)))
    return avg_sd
...
# ----------------------------
# Federated training main (with FedBN)
# ----------------------------
def main():
    print("DEVICE:", DEVICE)
    ...
    for r in range(COMM_ROUNDS):
        ...
        # Save a snapshot of global BN keys to keep BN local during aggregation
        global_state_for_bn = {k: v.cpu() for k, v in global_model.state_dict().items()}
        ...
        for i, client in enumerate(per_client_dataloaders):
            ...
            # Move local model to CPU for aggregation to save GPU mem, and keep BN parameters (they will remain local)
        ...
        # Aggregate via FedBN-aware weighted averaging
        print("\nAggregating local models with FedBN (only non-BN params averaged)")
        ...
        avg_state = fedbn_average_models(local_models, norm_weights, global_state_for_bn)
        # Load aggregated state into global model (keep BN from before)
