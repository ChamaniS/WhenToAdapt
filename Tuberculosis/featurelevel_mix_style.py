# fl_train_mixstyle_harmonize_singlehist_perclass.py
"""
FedAvg + feature-level MixStyle + harmonized-sample saving with combined grayscale histogram.
Saves per-round:
 - comparison grid per-client (top row: originals, mid row: harmonized, bottom row: grayscale hist overlay)
 - global test accuracy plot
 - global test loss plot
 - per-client test accuracy plot
 - per-client test loss plot

Prints per-client **per-class** metrics in a human-friendly table (no CSVs saved).
Includes a reference to the uploaded sample image (local path) if you want to inspect it:
...
# Path to the uploaded sample image (developer-provided). Kept here for convenience.
UPLOADED_SAMPLE_PATH = "/mnt/data/bcea4669-5b03-46c7-9fe6-f80536ff4b98.png"

# -----------------------
# USER CONFIG (edit to your paths & preferences)
# -----------------------
CLIENT_ROOTS = [
...
ARCH = "densenet169"        # torchvision model name (keep as-is or change)
PRETRAINED = True
USE_MIXSTYLE = True
MIX_P = 0.5
MIX_ALPHA = 0.1
...
# how many sample images per client to save in comparison grid
N_SAMPLES = 7

# filenames for round-series plots
...
# -----------------------
# Harmonized saving helpers (single grayscale hist)
# -----------------------
...
def save_comparison_grid_singlehist(client_idx, client_entry, round_num, out_base=OUTPUT_DIR, n_samples=N_SAMPLES):
    ...
    for s_i in idxs:
        ...
        try:
            ...
            continue
        orig_arr = np.array(pil).astype(np.uint8)  # HxWx3
        # harmonized (apply val_tf then unnormalize)
        if val_tf is None:
            print("[HARM] No val transform provided; skipping.")
            continue
        t = val_tf(pil)  # tensor CxHxW normalized
        harm_arr = unnormalize_tensor_to_uint8(t)  # HxWx3 uint8
        # resize both to same dims (use harm dims)
        ...
        # compute grayscale histograms (normalized)
    ...
    ncol = len(orig_imgs)
    # build figure grid: 3 x ncol
    fig_h = 3 * (IMG_SIZE / 72)  # rough scaling
    ...
    for j in range(ncol):
        ...
        # bottom: overlay grayscale histograms (orig = blue, harm = orange)
...
# -----------------------
# Plot helpers for per-round series
# -----------------------
def save_round_series_plots(round_results, per_client_acc_history, per_client_loss_history, out_dir=OUTPUT_DIR):
    ...
    # global test accuracy & loss must be present in round_results
...
# -----------------------
# Pretty print per-class metrics (formatting like your example)
# -----------------------
...
def main():
    ...
    # clients now contains only clients that had train samples
    ...
    for r in range(COMM_ROUNDS):
        ...
        # compute combined class weights
        ...
        # Per-client test + save harmonized comparison grid
        ...
        for i, client in enumerate(clients):
            ...
            # request per-class metrics here
            ...
            # pretty print per-client summary + per-class table
            pretty_print_per_class_metrics(i, client.get("name"), cl_metrics)

            # append per-client series history (for plots)
            ...
            # Save harmonized sample comparison grid with single grayscale hist per column
            try:
        ...
        # Save checkpoint (model state + metrics) - no CSVs
        ckpt = {
            "round": r+1,
        ...
        # append round summary and save per-round series plots
