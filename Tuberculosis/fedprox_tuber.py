...
with optional FedProx-style proximal penalty added to local objectives.
...
# FedProx hyperparameter: proximal penalty. Set 0.0 to get FedAvg behavior.
FEDPROX_MU = 0.1
# ----------------------------
...
def train_local(model,
    ...
    """
    Local training supporting FedProx-style proximal penalty.
    - fedprox_mu: mu hyperparameter (0.0 means no proximal penalty i.e. FedAvg)
    - global_state_dict: state_dict of the global model (cpu tensors OK). If provided,
      we compute proximal penalty sum_k || param_k - global_param_k ||^2.
    """
    model.to(device)
    scaler = torch.cuda.amp.GradScaler() if (use_amp and device.type == "cuda") else None

    # Prepare global parameters on device for proximal penalty (if provided)
    global_params = None
    if fedprox_mu and global_state_dict is not None:
        global_params = {}
        for k, v in global_state_dict.items():
            try:
                global_params[k] = v.to(device)
            except Exception:
                # fallback: clone then to device
                global_params[k] = v.clone().to(device)
        ...
        for x, y in pbar:
            ...
            with torch.cuda.amp.autocast(enabled=(scaler is not None)):
                ...
                if fedprox_mu and global_params is not None:
                    ...
                    # accumulate squared distance over parameters using named_parameters()
                    for name, p in model.named_parameters():
                        g = global_params.get(name, None)
                        if g is None:
                            # try 'module.' prefix fallback (in case of DataParallel naming differences)
                            g = global_params.get("module." + name, None)
                        if g is not None:
                            # p requires_grad=True, g does not; difference produces a tensor grad-compatible
                            prox_loss = prox_loss + (p - g).pow(2).sum()
                    # scale by mu/2 in final loss
                    prox_loss = 0.5 * float(fedprox_mu) * prox_loss

                loss = ce_loss + (prox_loss if prox_loss is not None else 0.0)
...
def main():
    ...
    for r in range(COMM_ROUNDS):
        ...
        for i, client in enumerate(per_client_dataloaders):
            ...
            # prepare a CPU copy of global state_dict to pass in (we'll move to device inside train_local)
            # Use .cpu().clone() to ensure tensors are CPU and safe to move to device later
            global_state = {k: v.cpu().clone() for k, v in global_model.state_dict().items()}

            # local training using FedProx proximal penalty
            logs = train_local(local_model,
                               client['train'],
                               criterion,
                               optimizer,
                               DEVICE,
