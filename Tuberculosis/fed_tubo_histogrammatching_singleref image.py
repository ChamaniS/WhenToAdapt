# fl_train_histmatch_auto_ref.py
"""
Federated averaging (FedAvg) across clients for TB CXR classification,
with histogram-matching harmonization using a single reference image.

Reference image selection:
  - If USE_AUTO_SELECT_REF=True, the script picks the client with the largest
    number of training images and uses a single image from that client's train
    folder (median-index file) as the reference.
  - If USE_AUTO_SELECT_REF=False, set REFERENCE_IMAGE_PATH to a single image file.

This variant:
  - prints class-wise metrics (per-class precision/recall/f1/specificity/support)
    in a readable table after each communication round for both combined and
    per-client test sets.
  - avoids re-creating harmonized folders / files when they already exist.
  - reuses a saved reference image if present.
"""
import os
...
USE_HIST_MATCH = True                 # apply histogram matching and use matched copies
USE_AUTO_SELECT_REF = True            # if True: auto-find client with most train images and pick single file
REFERENCE_IMAGE_PATH = None           # if USE_AUTO_SELECT_REF==False, set this to a single image path
REF_RESIZE = (IMG_SIZE, IMG_SIZE)     # resized shape for reference image
SAVE_HIST_QA = True                   # save small QA visualizations for each client (grid + hist)
...
# ----------------------------
# Histogram-matching using a single reference image chosen automatically
# ----------------------------
def select_reference_client_and_image(client_roots: List[str], class_names: List[str]) -> Tuple[int, str]:
    counts = []
    train_lists = []
    for cr in client_roots:
        train_dir = os.path.join(cr, "train")
        files = []
        if os.path.isdir(train_dir):
            for cls in sorted(os.listdir(train_dir)):
                ...
                if not os.path.isdir(cls_dir):
                    continue
                files.extend([os.path.join(cls_dir, f) for f in sorted(os.listdir(cls_dir)) if f.lower().endswith((".png",".jpg",".jpeg"))])
        counts.append(len(files))
        train_lists.append(sorted(files))
    if sum(counts) == 0:
        raise RuntimeError("No training images found in any client train/ folders.")
    best_idx = int(np.argmax(counts))
    best_files = train_lists[best_idx]
    if len(best_files) == 0:
        raise RuntimeError(f"No train images found in chosen client {client_roots[best_idx]}")
    mid = len(best_files) // 2
    selected = best_files[mid]
    return best_idx, selected

def load_reference_image_from_file(ref_image_path: str, resize_to=(224,224)):
...
def histogram_match_dataset(src_split_dir: str, dst_split_dir: str, reference_image: np.ndarray):
    ...
    for cls in sorted(os.listdir(src_split_dir)):
        ...
        for fn in sorted(os.listdir(src_cls)):
            if not fn.lower().endswith((".png",".jpg",".jpeg",".bmp",".tif",".tiff")):
                continue
            src_p = os.path.join(src_cls, fn)
            dst_p = os.path.join(dst_cls, fn)
...
def _client_dst_has_files(dst_root):
    """Return True if dst_root already contains any train/val/test image files (i.e. harmonized appears present)."""
...
def create_histogram_matched_client_roots_single_ref_auto(client_roots, client_names, out_base, resize_ref=(224,224), use_auto_select=True, explicit_ref_path=None):
    base = os.path.join(out_base, "HistMatch")
    os.makedirs(base, exist_ok=True)
    ref_save = os.path.join(base, "reference_image.png")

    # If reference exists, reuse it (and don't recompute selection)
    if os.path.exists(ref_save):
        try:
            ...
            ref_client_idx = None
            ref_basename = os.path.basename(ref_save)
            ref_info = (ref_client_idx, ref_basename)
        except Exception:
            os.remove(ref_save)
            ref_img = None
            ref_info = (None, None)
    ...
    if ref_img is None:
        if use_auto_select:
            ref_client_idx, ref_img_path = select_reference_client_and_image(client_roots, CLASS_NAMES)
            print(f"[HistMatch] Auto selected client {client_names[ref_client_idx]} (index {ref_client_idx}) with reference image: {ref_img_path}")
        else:
            if explicit_ref_path is None:
                raise ValueError("explicit_ref_path must be provided when use_auto_select=False")
            ref_img_path = explicit_ref_path
            ref_client_idx = None
            for i, cr in enumerate(client_roots):
                if os.path.abspath(ref_img_path).startswith(os.path.abspath(cr)):
                    ref_client_idx = i
                    break
            print(f"[HistMatch] Using explicit reference image: {ref_img_path} (client idx {ref_client_idx})")
        ref_img = load_reference_image_from_file(ref_img_path, resize_to=resize_ref)
        Image.fromarray(ref_img).save(ref_save)
        print(f"[HistMatch] Saved resized reference image to: {ref_save}")
        ref_info = (ref_client_idx, os.path.basename(ref_img_path))

    hm_roots = []
    for i, cr in enumerate(client_roots):
        cname = client_names[i] if i < len(client_names) else f"client_{i}"
        dst = os.path.join(base, cname)
        # if destination already has files, skip harmonization for this client
        if _client_dst_has_files(dst):
            ...
            hm_roots.append(dst)
            continue
        # create dst and run harmonization (skip individual files if already exist)
        os.makedirs(dst, exist_ok=True)
        print(f"[HistMatch] Harmonizing client {cname} -> {dst} ...")
        for split in ("train", "val", "test"):
            src_split = os.path.join(cr, split)
            dst_split = os.path.join(dst, split)
            if not os.path.isdir(src_split):
                continue
            histogram_match_dataset(src_split, dst_split, ref_img)
        hm_roots.append(dst)

    print("[HistMatch] Done creating histogram-matched datasets using single reference image.")
    return hm_roots, ref_save, ref_info
...
def save_hist_match_examples(hm_client_root, orig_client_root, dest_dir, n_samples=7):
    ...
    if os.path.exists(out_path):
        print(f"[HistMatch QA] QA image already exists at {out_path}, skipping creation.")
        return
    ...
    for cls in sorted(os.listdir(orig_train)):
        ...
        if not os.path.isdir(clsdir): continue
    ...
    print(f"[HistMatch QA] Saved visual QA to {out_path}")

# ----------------------------
# Pretty printing helper for per-class metrics
# ----------------------------
def _fmt_pct(x):
...
def print_metrics_summary(metrics: dict, class_names: list, title: str = "Test"):
    if not metrics:
        print(f"{title}: no samples -> skipping metrics print.")
        return
    print()
...
def main():
    ...
    # Optionally create histogram-matched copies and use those roots
    if USE_HIST_MATCH:
        try:
            if USE_AUTO_SELECT_REF:
                hm_roots, ref_img_path, ref_info = create_histogram_matched_client_roots_single_ref_auto(
                    CLIENT_ROOTS, CLIENT_NAMES, OUTPUT_DIR, resize_ref=REF_RESIZE, use_auto_select=True
                )
                print(f"[Main] Reference info: {ref_info}")
            else:
                if REFERENCE_IMAGE_PATH is None:
                    raise ValueError("REFERENCE_IMAGE_PATH must be set if USE_AUTO_SELECT_REF is False")
                hm_roots, ref_img_path, ref_info = create_histogram_matched_client_roots_single_ref_auto(
                    CLIENT_ROOTS, CLIENT_NAMES, OUTPUT_DIR, resize_ref=REF_RESIZE, use_auto_select=False, explicit_ref_path=REFERENCE_IMAGE_PATH
                )
    ...
    # Save QA visuals per client (optional)
    if USE_HIST_MATCH and SAVE_HIST_QA:
        qa_dir = os.path.join(OUTPUT_DIR, "HistMatch_QA")
        os.makedirs(qa_dir, exist_ok=True)
        for i, orig_root in enumerate(CLIENT_ROOTS):
            cname = CLIENT_NAMES[i] if i < len(CLIENT_NAMES) else f"client_{i}"
            hm_root = client_roots_used[i]
            dest = os.path.join(qa_dir, cname)
            try:
                save_hist_match_examples(hm_root, orig_root, dest, n_samples=7)
            except Exception as e:
    ...
    for r in range(COMM_ROUNDS):
        ...
        # Global validation on combined val
        ...
        # Print combined/global test metrics with class-wise breakdown
        ...
        # Per-client testing & CSV saving (with class-wise prints)
        ...
        # Update simple plots for global test accuracy & loss rounds
