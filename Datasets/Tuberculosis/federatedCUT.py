...
def PatchNCELoss (f_q ,f_k ,tau =PATCHNCE_TAU ):
    ...
    l_pos_unsq =l_pos .unsqueeze (2 )
    logits =torch .cat ([l_pos_unsq ,l_neg ],dim =2 )
    logits =logits /tau 
...
def sample_patch_indices (spatial ,S ,device ):
    ...
    return [torch .randperm (spatial ,device =device )[:S ]for _ in range (1 )]
...
def gather_patches_by_indices (feat ,indices ):
    ...
    for b in range (B ):
        idx =indices [b ]
        out_list .append (feat [b :b +1 ,:,idx ])
