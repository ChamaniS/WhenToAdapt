...
def main ():
    ...
    c_global =zeros_like_state_dict_on_cpu (initial_sd )
    c_clients =[zeros_like_state_dict_on_cpu (initial_sd )for _ in range (len (per_client_dataloaders ))]
    ...
    for r in range (COMM_ROUNDS ):
        ...
        for i ,client in enumerate (per_client_dataloaders ):
            ...
            if not SCAFFOLD :
                ...
                local_model =trained_local_model 
        ...
        if not SCAFFOLD :
            ...
            if n_clients <=0 :
                ...
                if len (local_delta_cs )>0 :
                    sum_delta_c ={k :torch .zeros_like (v .cpu ())for k ,v in c_global .items ()}
