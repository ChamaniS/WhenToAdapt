...
BATCH_CUT =1 
CUT_LAYERS_SAMPLE =5 
PATCHNCE_SAMPLES =256 
PATCHNCE_TAU =0.07 
CUT_IDENTITY =True 
CUT_ID_WEIGHT =1.0 
CUT_X_WEIGHT =1.0 if CUT_IDENTITY else 10.0 
CUT_LR =1e-4 
CUT_BETA =(0.5 ,0.999 )
...
class CutResnetGenerator (nn .Module ):
    def __init__ (self ,in_ch =3 ,out_ch =3 ,ngf =64 ,nblocks =6 ):
        ...
        self .encoder_initial =nn .Sequential (*enc )
    ...
    def encode (self ,x ):
        ...
        if len (self .res_blocks )>0 :
            ...
            rest_blocks =nn .Sequential (*list (self .res_blocks .children ())[1 :])if len (self .res_blocks )>1 else None 
            out_first =first_block (out )
            feats .append (out_first )
            if rest_blocks is not None :
...
def sample_patches_from_feature_map (feat ,n_patches =PATCHNCE_SAMPLES ):
    if feat is None :
        raise ValueError ("feat is None in sample_patches_from_feature_map")
    if feat .dim ()==4 :
        B ,C ,H ,W =feat .shape 
        spatial =H *W 
        feat_flat =feat .view (B ,C ,spatial )
    elif feat .dim ()==3 :
        B ,C ,spatial =feat .shape 
        feat_flat =feat 
    else :
        raise ValueError (f"Unexpected feat.dim()={feat.dim()}; expected 3 or 4 dims")
    ...
    for b in range (B ):
        idx =indices [b ]
        out_list .append (feat_flat [b :b +1 ,:,idx ])
    ...
    for epoch in range (epochs ):
        ...
        for real_A in loop :
            ...
            if CUT_IDENTITY :
                ...
                feats_GB =G_A2B .encode (fake_B .detach ())
                for l in range (CUT_LAYERS_SAMPLE ):
                    if l >=len (feats_B ):break 
                    fB =feats_B [l ];fGB =feats_GB [l ]
                    key =f"l{l}"
                    if key not in projectors :
                        projectors [key ]=ProjectorMLP (in_ch =fB .shape [1 ]).to (device )
                    H =projectors [key ]
                    pB =H (fB );pGB =H (fGB )
                    pB =pB .view (pB .shape [0 ],pB .shape [1 ],-1 )
                    pGB =pGB .view (pGB .shape [0 ],pGB .shape [1 ],-1 )
                    pB =nn .functional .normalize (pB ,dim =1 )
                    pGB =nn .functional .normalize (pGB ,dim =1 )
                    f_q ,idxs_q =sample_patches_from_feature_map (pGB ,n_patches =PATCHNCE_SAMPLES )
                    f_k ,_ =sample_patches_from_feature_map (pB ,n_patches =PATCHNCE_SAMPLES )
                    loss_id +=PatchNCELoss (f_q ,f_k ,tau =PATCHNCE_TAU )
            ...
            for l in range (CUT_LAYERS_SAMPLE ):
                ...
                fA =feats_A [l ];fFake =feats_fakeB [l ]
                key =f"l{l}"
                if key not in projectors :
                    projectors [key ]=ProjectorMLP (in_ch =fA .shape [1 ]).to (device )
                H =projectors [key ]
                pA =H (fA );pFake =H (fFake )
                pA =pA .view (pA .shape [0 ],pA .shape [1 ],-1 )
                pFake =pFake .view (pFake .shape [0 ],pFake .shape [1 ],-1 )
                pA =nn .functional .normalize (pA ,dim =1 )
                pFake =nn .functional .normalize (pFake ,dim =1 )
                f_q ,idxs_q =sample_patches_from_feature_map (pFake ,n_patches =PATCHNCE_SAMPLES )
                f_k ,_ =sample_patches_from_feature_map (pA ,n_patches =PATCHNCE_SAMPLES )
                loss_patchnce +=PatchNCELoss (f_q ,f_k ,tau =PATCHNCE_TAU )
            ...
            loop .set_postfix ({"loss_D":loss_D .item (),"loss_G_GAN":loss_G_GAN .item (),"loss_patchnce":float (loss_patchnce ),"loss_id":float (loss_id )})
...
def make_explicit_comparison_grid_for_file (original_root :str ,harmonized_root :str ,client_name :str ,desired_fname :str ,out_base :str ,img_size =IMG_SIZE ,amp_factor =DIFF_AMP_FACTOR ):
    out_dir =os .path .join (out_base ,"ExplicitComparison")
    ensure_dir (out_dir )
    out_path =os .path .join (out_dir ,f"explicit_comparison_{client_name}.png")

    splits =["train","val","test"]


    orig_path =None 
    orig_found_split =None 
    for split in splits :
        split_dir =os .path .join (original_root ,split )
        if os .path .isdir (split_dir ):
            p =_find_file_recursive (split_dir ,desired_fname )
            if p :
                orig_path =p 
                orig_found_split =split 
                break 

    hm_path =None 
    hm_found_split =None 
    for split in splits :
        split_dir =os .path .join (harmonized_root ,split )
        if os .path .isdir (split_dir ):
            p =_find_file_recursive (split_dir ,desired_fname )
            if p :
                hm_path =p 
                hm_found_split =split 
                break 

    if orig_path is None :
        print (f"[EXPL-VIS] original file '{desired_fname}' not found under any of {', '.join(os.path.join(original_root, s) for s in splits)} for {client_name}. Skipping.")
        return 
    else :
        print (f"[EXPL-VIS] Found original file in '{orig_found_split}' split: {orig_path}")

    if hm_path is None :
        print (f"[EXPL-VIS] harmonized file '{desired_fname}' not found under any of {', '.join(os.path.join(harmonized_root, s) for s in splits)} for {client_name}. Skipping.")
        return 
    else :
        print (f"[EXPL-VIS] Found harmonized file in '{hm_found_split}' split: {hm_path}")


    try :
        pil_o =Image .open (orig_path ).convert ("RGB").resize ((img_size ,img_size ),resample =Image .BILINEAR )
        pil_h =Image .open (hm_path ).convert ("RGB").resize ((img_size ,img_size ),resample =Image .BILINEAR )
        orig_arr =np .array (pil_o ).astype (np .uint8 )
        hm_arr =np .array (pil_h ).astype (np .uint8 )

        orig_gray =(0.2989 *orig_arr [:,:,0 ].astype (np .float32 )+
        0.5870 *orig_arr [:,:,1 ].astype (np .float32 )+
        0.1140 *orig_arr [:,:,2 ].astype (np .float32 ))
        hm_gray =(0.2989 *hm_arr [:,:,0 ].astype (np .float32 )+
        0.5870 *hm_arr [:,:,1 ].astype (np .float32 )+
        0.1140 *hm_arr [:,:,2 ].astype (np .float32 ))

        diff =np .abs (orig_gray -hm_gray )
        ...
        axs [0 ].imshow (orig_arr );axs [0 ].axis ('off');axs [0 ].set_title ("Original (resized)",fontsize =10 )
        axs [1 ].imshow (hm_arr );axs [1 ].axis ('off');axs [1 ].set_title ("Harmonized (CUT)",fontsize =10 )
        axs [2 ].imshow (amplified ,cmap ='gray',vmin =0 ,vmax =255 );axs [2 ].axis ('off');axs [2 ].set_title (f"Amplified abs gray diff (x{amp_factor})",fontsize =10 )
        plt .suptitle (f"{client_name}: {os.path.basename(orig_path)}",fontsize =11 )
        plt .tight_layout (rect =[0 ,0.03 ,1 ,0.95 ])
        plt .savefig (out_path ,dpi =150 ,bbox_inches ='tight');plt .close (fig )
        print (f"[EXPL-VIS] Saved explicit comparison for {client_name} -> {out_path}")
    except Exception as e :
        print (f"[EXPL-VIS] Failed explicit comparison for {client_name}: {e}")
...
def save_round_series_plots (round_results ,per_client_test_acc_history ,out_dir =OUT_DIR ):
...
def main ():
    ...
    combined_loaders ,combined_sizes ,class_names ,combined_train_ds ,per_client_dataloaders ,per_client_test_dsets =make_multi_client_dataloaders_from_roots (harmonized_roots ,batch_size =BATCH_SIZE ,image_size =IMG_SIZE ,workers =WORKERS ,pin_memory =PIN_MEMORY and (DEVICE =="cuda"))
    ...
    explicit_out =os .path .join (visuals_out ,"ExplicitComparison")
    ensure_dir (explicit_out )
    for i ,client_root in enumerate (CLIENT_ROOTS ):
        cname =CLIENT_NAMES [i ]
        desired =COMPARE_FILES .get (cname )
        if desired :
            make_explicit_comparison_grid_for_file (client_root ,harmonized_roots [i ],cname ,desired ,visuals_out ,img_size =IMG_SIZE ,amp_factor =DIFF_AMP_FACTOR )
        else :
            print (f"[EXPL-VIS] No mapping for client {cname}, skipping explicit comparison.")
    ...
    print (f"Explicit comparison grids stored under: {os.path.join(OUT_DIR, 'CUT_Harmonized_Visuals', 'ExplicitComparison')}")
    print ("Global plots:")
