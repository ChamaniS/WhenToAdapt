...
def load_reference_image (reference_client_idx =0 ,reference_image_path =None ,resize_to =(224 ,224 )):
    if reference_image_path is not None and os .path .exists (reference_image_path ):
        ...
        if len (files )==0 :
            raise ValueError (f"No images found in reference directory: {ref_img_dir}")
...
resize_ref =(224 ,224 )):
    ...
    ref_img =load_reference_image (reference_client_idx ,reference_image_path ,resize_to =resize_ref )
    ...
    for i ,cname in enumerate (client_names ):
        ...
        if histmatched_split_ready (train_dst_img ,train_dst_mask )and histmatched_split_ready (val_dst_img ,val_dst_mask )and histmatched_split_ready (test_dst_img ,test_dst_mask ):
...
def plot_metrics (round_metrics ,out_dir ):
    ...
    plt .legend ()
    plt .tight_layout ()
    plt .savefig (os .path .join (out_dir ,"dice_no_bg_histmatch_ducknetbs4.png"))
    plt .close ()
    ...
    plt .legend ()
    plt .tight_layout ()
    plt .savefig (os .path .join (out_dir ,"iou_no_bg_histmatch_ducknetbs4.png"))
    plt .close ()
    ...
    plt .legend ()
    plt .tight_layout ()
    plt .savefig (os .path .join (out_dir ,"global_dice_no_bg_histmatch_ducknetbs4.png"))
    plt .close ()
    ...
    plt .legend ()
    plt .tight_layout ()
    plt .savefig (os .path .join (out_dir ,"global_iou_no_bg_histmatch_ducknetbs4.png"))
    plt .close ()
