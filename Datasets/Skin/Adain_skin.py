...
out_dir ="Outputs_AdaIN_SkinLesion"
...
def create_selected_comparison_grids (round_idx ,original_train_dirs ,harmonized_train_dirs ,selected_targets ,out_base_dir =out_dir ):
    for i ,cname in enumerate (client_names ):
        targets =selected_targets .get (cname ,[])
        if len (targets )==0 :
            continue 

        orig_dir =original_train_dirs [i ]
        hm_dir =harmonized_train_dirs [i ]
