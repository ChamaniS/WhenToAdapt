...
out_dir = "Outputs_inputlvlbreast"
os.makedirs(out_dir, exist_ok=True)

# Input-level MixStyle settings
MIXSTYLE_P = 1.0
MIXSTYLE_ALPHA = 0.2
...
# -------------------------
# Input-level MixStyle
# -------------------------
def input_mixstyle(images, p=1.0, alpha=0.2, eps=1e-6, force_pair=False, fixed_lam=None):
    """
    MixStyle on input images (B x C x H x W).

    force_pair=True  -> avoids self-pairing in visualization
    fixed_lam=None   -> sampled from Beta(alpha, alpha)
    fixed_lam=0.5    -> deterministic and easier to visualize
    """
...
def _rgb_abs_diff_uint8(orig_u8, mix_u8, amp=3.0):
    """
    Compute absolute RGB difference in pixel space, then amplify for display.
    """
...
def find_image_and_mask_for_client(client_name, target_filename):
    """
    Searches train/val/test image folders for the exact filename (case-insensitive)
    and returns (img_path, mask_path, split_name).
    """
...
def save_input_mixstyle_grid(selected_items, transform, out_base, grid_name="input_mixstyle_four_dataset_comparison.png"):
    """
    Saves a 3-row grid:
      row 1: original image
      row 2: MixStyle output image
      row 3: amplified absolute RGB difference

    The difference is computed in pixel space, not on the normalized tensor.
    """
    ensure_dir(out_base)
    ...
    # Force visible mixing for visualization only
    mixed = input_mixstyle(
        batch,
        p=1.0,
    ...
    fig.suptitle("Input-level MixStyle Comparison Across 4 Skin Lesion Datasets", fontsize=16, y=0.98)
    row_labels = ["Original", "MixStyle", "Amplified |Difference|"]
    for r in range(3):
        fig.text(0.01, 0.82 - r * 0.31, row_labels[r], fontsize=12, va='center', rotation='vertical')

    for i in range(n):
        orig_img = _tensor_to_uint8_rgb(originals[i])
        mix_img = _tensor_to_uint8_rgb(mixed_cpu[i])
        diff_img = _rgb_abs_diff_uint8(orig_img, mix_img, amp=3.0)

        axs[0, i].imshow(orig_img)
        axs[0, i].axis("off")
        axs[0, i].set_title(titles[i], fontsize=9)

        axs[1, i].imshow(mix_img)
        axs[1, i].axis("off")
    ...
    print(f"Saved input-level MixStyle comparison grid to: {out_path}")
...
def train_local(loader, model, loss_fn, opt):
    ...
    for _ in range(LOCAL_EPOCHS):
        for batch in tqdm(loader, leave=False):
            ...
            # Apply input-level MixStyle to every training batch
            data = input_mixstyle(data, p=MIXSTYLE_P, alpha=MIXSTYLE_ALPHA)
...
def plot_metrics(round_metrics, out_dir):
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(out_dir, "dice_no_bg_inputmixstyle.png"))
    plt.close()
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(out_dir, "iou_no_bg_inputmixstyle.png"))
    plt.close()
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(out_dir, "global_dice_no_bg_inputmixstyle.png"))
    plt.close()
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(out_dir, "global_iou_no_bg_inputmixstyle.png"))
    plt.close()
...
def main():
    ...
    # Save only one input-level MixStyle comparison grid for these 4 images
    selected_items = [
        {"client_name": "BUSBRA",   "filename": "0001-r.png"},
        {"client_name": "BUS",      "filename": "00104.png"},
        {"client_name": "BUSI", "filename": "101.png"},
        {"client_name": "UDIAT", "filename": "000007.png"},
    ]

    save_input_mixstyle_grid(
        selected_items=selected_items,
        transform=val_tf,
        out_base=os.path.join(out_dir, "MixStyleSamples"),
        grid_name="input_mixstyle_four_dataset_comparison.png"
    )
