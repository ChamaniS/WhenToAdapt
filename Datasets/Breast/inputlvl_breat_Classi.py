...
OUTPUT_DIR = "breast_classi_inputlevel"
MODEL_NAME = "efficientnet_b0_breast_tumor_fedavg_mixstyle.pth"
WEIGHTS_PATH = r"/lustre06/project/6008975/csj5/narvalenv/pretrained/efficientnet_b0_rwightman-7f5810bc.pth"

output_file = r"/lustre06/project/6008975/csj5/narvalenv/inputlvl_breast_classi.txt"
...
def tensor_to_uint8_display(t: torch.Tensor, mean_vals=mean, std_vals=std):
    img = tensor_to_display_image(t, mean_vals=mean_vals, std_vals=std_vals)
...
def build_model(num_classes: int):
    ...
    model = load_pretrained_weights(model, WEIGHTS_PATH, device=DEVICE)
    return model
    ...
    for r in range(COMM_ROUNDS):
        ...
        for client_name in CLIENT_NAMES:
            ...
            for ep in range(LOCAL_EPOCHS):
                ...
                loop = tqdm(train_loader, desc=f"{client_name} | Epoch {ep + 1}/{LOCAL_EPOCHS}", leave=False)
                ...
                for images, labels in loop:
                    ...
                    try:
                        images = input_mixstyle(images, p=MIXSTYLE_P, alpha=MIXSTYLE_ALPHA, eps=MIXSTYLE_EPS)
                    except Exception:
                        pass
