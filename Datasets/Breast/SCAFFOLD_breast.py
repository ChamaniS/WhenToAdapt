...
out_dir = "Outputs_scaffold_breast"
os.makedirs(out_dir, exist_ok=True)

# -------------------------
# SCAFFOLD hyperparameters
# -------------------------
USE_SGD = True
SGD_LR = 1e-3
ADAMW_LR = 1e-4
...
# Loss balancing for strong foreground imbalance in skin lesion segmentation
FOREGROUND_BCE_WEIGHT = 5.0   # try 5.0 first; if still under-segmenting, try 8.0 or 10.0
DICE_WEIGHT = 1.0
BCE_WEIGHT = 1.0
...
# -------------------------
# Stronger loss for imbalanced segmentation
# -------------------------
...
def average_model_state_dict(models, weights):
    """
    Average the entire state dict across models.
    For floating tensors: weighted average.
    For integer buffers like num_batches_tracked: copy from the first model.
    """
...
def train_local_scaffold(loader, model, loss_fn, opt, global_c, client_c, round_idx=0, client_idx=0):
    ...
    SCAFFOLD local training:
        grad <- grad + (c - c_i)
        c_i_new = c_i - c + (w_init - w_final) / (eta * K)
    """
    model.train()
    total_loss = 0.0
    metrics = []
