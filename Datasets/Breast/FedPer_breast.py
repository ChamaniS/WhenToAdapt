...
# ============================================================
# FedPer for skin lesion segmentation
# Shared parts are aggregated across clients.
# Personalized parts stay local to each client.
# Adjust SHARED_KEYWORDS / PERSONAL_KEYWORDS if your UNet names differ.
# ============================================================
...
# FedPer layer selection rules
# Shared = aggregated globally
# Personal = kept local on each client
...
def ensure_mask_shape(target):
    """
    Converts masks to shape [B,1,H,W] if needed.
    Albumentations/ToTensorV2 can return [B,H,W] or [B,1,H,W] depending on pipeline.
    """
    if target.ndim == 3:
...
def infer_shared_keys(model):
    """
    Pick keys to be aggregated globally.
    Everything matching SHARED_KEYWORDS and not matching PERSONAL_KEYWORDS is treated as shared.
    """
    keys = []
...
def weighted_average_shared_states(shared_states, weights):
    """
    shared_states: list of dicts {key: tensor}
...
def main():
    ...
    # Use your UNET constructor here.
    # This matches your baseline code.
    ...
    # One personalized model per client
    ...
    for r in range(COMM_ROUNDS):
        ...
        # Sync global shared weights into each client before local training
