...
# Logging
# =========================================================
output_file = r"/lustre06/project/6008975/csj5/narvalenv/hismat_single_breast.txt"
...
OUTPUT_DIR = "breast_tumor_hismat_sin"
MODEL_NAME = "efficientnet_b0_breast_tumor_hismat_single.pth"
...
def first_image_in_imagefolder_root(root_dir):
    files = image_list_in_dir(root_dir)
    if len(files) == 0:
        raise ValueError(f"No images found in {root_dir}")
    return files[0]

def load_reference_image(reference_client_dir, reference_image_path=None, resize_to=(224, 224)):
    if reference_image_path is not None:
        if not os.path.exists(reference_image_path):
            raise FileNotFoundError(f"REFERENCE_IMAGE_PATH not found: {reference_image_path}")
        img = Image.open(reference_image_path).convert("RGB")
    else:
        first_path = first_image_in_imagefolder_root(reference_client_dir)
...
def main():
    ...
    # Pick the client with the highest number of training samples as reference
    ...
    reference_client_dir = paths[reference_client_name]["train"]

    # Use one representative image from the largest training dataset
    reference_rgb = load_reference_image(
        reference_client_dir,
    ...
    print(f"\nReference image saved to: {ref_save_path}")
