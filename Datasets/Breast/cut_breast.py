...
# Client 0 is used as the reference domain for CUT
reference_idx = 0
...
RESUME_CUT_IF_EXISTS = True
REGENERATE_HARMONIZED_DATA = True
RESUME_FEDAVG_IF_EXISTS = True
...
# FedAvg resume checkpoint
fedavg_latest_ckpt = os.path.join(out_dir, "global_model_latest.pth")
...
def find_mask_by_stem(mask_dir, stem, mask_exts):
    ...
    # some datasets use alternate names
...
def folder_has_files(folder):
    return os.path.isdir(folder) and len(list_image_files(folder)) > 0
...
class CUTGenerator(nn.Module):
    ...
    def __init__(self, in_ch=3, ngf=64):
        ...
        self.enc2 = conv_block(ngf, ngf * 2, down=True)      # /2
        self.enc3 = conv_block(ngf * 2, ngf * 4, down=True)  # /4
        self.enc4 = conv_block(ngf * 4, ngf * 8, down=True)  # /8
        self.enc5 = conv_block(ngf * 8, ngf * 8, down=True)  # /16
...
def sample_patches(feat_map, n_patches):
    ...
    flat = feat_map.view(B, C, -1)[:, :, idxs]   # [B,C,P]
    flat = flat.permute(0, 2, 1).reshape(-1, C)  # [B*P,C]
    return flat
...
# =========================================================
# CUT training and harmonization
# =========================================================
...
def main():
    ...
    # ------------------------------------------------------------------
    # Step 1: train or load CUT models for each non-reference client
    # ------------------------------------------------------------------
    cut_models = {}
    for i in range(NUM_CLIENTS):
        ...
        if RESUME_CUT_IF_EXISTS and os.path.exists(final_ckpt):
            ...
            print(f"[CUT] Training generator for {client_names[i]} -> {client_names[reference_idx]}")
            G = train_cut(
    ...
    for i in range(NUM_CLIENTS):
        ...
        if i == reference_idx:
            ...
            if REGENERATE_HARMONIZED_DATA and os.path.exists(dst_base):
                shutil.rmtree(dst_base, onerror=_on_rm_error)
    ...
    # ------------------------------------------------------------------
    # Step 3: FedAvg segmentation on harmonized data
    # ------------------------------------------------------------------
    global_model = UNET(in_channels=3, out_channels=1).to(DEVICE)

    if RESUME_FEDAVG_IF_EXISTS and os.path.exists(fedavg_latest_ckpt):
        print(f"[FedAvg] Loading latest global model checkpoint: {fedavg_latest_ckpt}")
        ck = torch.load(fedavg_latest_ckpt, map_location=DEVICE)
        if isinstance(ck, dict) and "model" in ck:
            global_model.load_state_dict(ck["model"])
        elif isinstance(ck, dict):
            global_model.load_state_dict(ck)
        else:
            global_model.load_state_dict(ck)
    ...
    for r in range(COMM_ROUNDS):
        ...
        # Save latest global checkpoint so you can resume after interruption
        torch.save({"model": global_model.state_dict(), "round": r + 1}, fedavg_latest_ckpt)
