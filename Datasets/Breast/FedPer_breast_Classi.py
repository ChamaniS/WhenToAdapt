...
OUTPUT_DIR = "breast_tumor_fedper"
MODEL_NAME = "efficientnet_b0_breast_tumor_fedper.pth"
...
def main():
    ...
    # Combined datasets only for reporting counts
    ...
    # Initialize a global shared model and personalized client models
    ...
    # Shared parameter snapshot from global model
    ...
    for r in range(COMM_ROUNDS):
        ...
        for i, client_name in enumerate(CLIENT_NAMES):
            ...
            # Start from this client's personalized model from previous round
            local_model = copy.deepcopy(client_models[client_name]).to(DEVICE)

            # Synchronize shared layers from the latest global shared model
        ...
        # Push new shared layers back to each client model, keep heads local
        for client_name in CLIENT_NAMES:
            load_shared_params(client_models[client_name], shared_state)

        # ---- Save best shared model by mean validation loss ----
        ...
        # ---- Per-client test evaluation after this round ----
        print("\n" + "=" * 30)
    ...
    # Save final shared model and all client models
    torch.save(
        {
            "shared_state": global_model.state_dict(),
            "client_states": {client: client_models[client].state_dict() for client in CLIENT_NAMES},
