...
# Optional: put reference filenames here if you want specific preview images.
# If a filename is None or not found, the first image in the train folder is used.
REFERENCE_IMAGES = {
    "Sartajbhuvaji": None,
    "rm1000": None,
    "thomasdubail": None,
...
# Harmonization / augmentation pipeline for training
...
# Preview transform for comparison grids (no normalization / tensor conversion)
...
def build_client_datasets(paths_dict, split, transform):
    ...
    for client in CLIENT_NAMES:
        ...
        ds = AlbumentationsImageFolder(split_dir, transform=transform)
        ds_list.append(ds)
...
def find_reference_image(train_root, preferred_name=None):
    if preferred_name is not None:
        candidate = os.path.join(train_root, preferred_name)
    ...
    raise FileNotFoundError(f"No image found in: {train_root}")

def save_harmonization_grid_for_client(client_name, image_path, out_dir):
    """
    Saves a 3-row comparison for one image:
    Row 1: original
    Row 2: harmonized
    Row 3: amplified absolute difference
    """
    original = load_rgb_image(image_path)
    harmonized = preview_tfms(image=original)["image"]

    if isinstance(harmonized, torch.Tensor):
        harmonized = harmonized.detach().cpu().permute(1, 2, 0).numpy()

    original_resized = A.Resize(IMG_SIZE, IMG_SIZE)(image=original)["image"]

    diff = np.abs(original_resized.astype(np.int16) - harmonized.astype(np.int16))
    ...
    plt.tight_layout()
    save_path = os.path.join(out_dir, f"{client_name}_harmonization_grid.png")
    plt.savefig(save_path, dpi=300, bbox_inches="tight")
    plt.close()

def save_overview_grid(paths, out_dir):
    """
    Saves a single 3x4 grid:
    Row 1 = originals
    Row 2 = harmonized
    Row 3 = amplified absolute differences
    """
    fig, axes = plt.subplots(3, len(CLIENT_NAMES), figsize=(5 * len(CLIENT_NAMES), 12))

    for col, client_name in enumerate(CLIENT_NAMES):
        train_root = paths[client_name]["train"]
        ref_name = REFERENCE_IMAGES.get(client_name, None)
        ref_path = find_reference_image(train_root, preferred_name=ref_name)

        original = load_rgb_image(ref_path)
        harmonized = preview_tfms(image=original)["image"]
        ...
        axes[0, col].set_title(f"{client_name}\nOriginal")
        axes[0, col].axis("off")

        axes[1, col].imshow(harmonized)
        axes[1, col].set_title("Harmonized")
        axes[1, col].axis("off")

        axes[2, col].imshow(diff)
        axes[2, col].set_title("Amplified diff")
        axes[2, col].axis("off")

    plt.tight_layout()
    plt.savefig(os.path.join(out_dir, "harmonization_overview_grid.png"), dpi=300, bbox_inches="tight")
    plt.close()
...
def main():
    ...
    # Save comparison grids before training
    save_overview_grid(paths, OUTPUT_DIR)
    for client in CLIENT_NAMES:
        ref_name = REFERENCE_IMAGES.get(client, None)
        ref_path = find_reference_image(paths[client]["train"], preferred_name=ref_name)
        save_harmonization_grid_for_client(client, ref_path, OUTPUT_DIR)
    ...
    print(os.path.join(OUTPUT_DIR, "harmonization_overview_grid.png"))
    for client in CLIENT_NAMES:
        print(os.path.join(OUTPUT_DIR, f"{client}_harmonization_grid.png"))
