...
# Local EfficientNet-B0 checkpoint path
    ...
    if isinstance(ds, ConcatDataset):
        ...
        for sub in ds.datasets:
            if hasattr(sub, "targets"):
                targets.extend(list(sub.targets))
            elif hasattr(sub, "samples"):
                targets.extend([s[1] for s in sub.samples])
            else:
                raise ValueError("Unsupported dataset type inside ConcatDataset.")
        return targets
...
def compute_class_weights_from_dataset(dataset, num_classes):
    targets = np.array(get_targets_from_dataset(dataset), dtype=np.int64)
...
def load_efficientnet_weights(model, weights_path):
    """
    Loads a local EfficientNet-B0 checkpoint into the model.
    Works with:
      - raw state_dict
      - checkpoint dict containing 'state_dict', 'model_state', or 'model_state_dict'
    It ignores classifier keys or any mismatched shapes.
    """
    if not os.path.isfile(weights_path):
        raise FileNotFoundError(
            f"Local EfficientNet checkpoint not found: {weights_path}\n"
            "Fix WEIGHTS_PATH before training."
        )
    ...
    print(f"[INFO] Loaded {len(matched)} tensors from: {weights_path}")
    if skipped:
        print(f"[INFO] Skipped {len(skipped)} unmatched tensors (expected for classifier or shape mismatch).")

    return model

def build_model(num_classes, weights_path=None):
    """
    Build EfficientNet-B0 without downloading anything from torchvision.
    Then load local checkpoint weights from WEIGHTS_PATH if provided.
    """
    model = efficientnet_b0(weights=None)  # no download, ever
...
def compute_specificity_from_cm(cm):
    ...
    macro_specificity = float(np.mean(per_class_specificity)) if per_class_specificity else 0.0
...
def average_state_dicts_weighted(models, weights):
    ...
    Non-floating tensors/buffers are copied from the first model.
    """
    ...
    for k in avg_sd.keys():
        ...
        if torch.is_floating_point(ref_tensor):
            ...
            avg_sd[k] = ref_tensor.detach().cpu().clone()

    return avg_sd

def fedprox_penalty(model, global_state_dict, device):
    """
    Computes sum_k || w_k - w_k^global ||^2 over matching named parameters.
    """
    if global_state_dict is None:
        return torch.tensor(0.0, device=device)

    penalty = torch.tensor(0.0, device=device)
    global_named = {name: tensor.to(device) for name, tensor in global_state_dict.items()}

    for name, p in model.named_parameters():
        g = global_named.get(name, None)
        if g is None:
            g = global_named.get("module." + name, None)
        if g is not None:
            penalty = penalty + (p - g).pow(2).sum()

    return penalty

def run_epoch(model, loader, criterion, optimizer=None, train=True, global_state_dict=None, fedprox_mu=0.0):
    if train:
        model.train()
    else:
        model.eval()
    ...
    for images, labels in loop:
        ...
        if train:
            ...
            with torch.cuda.amp.autocast(enabled=USE_AMP):
                ...
                if fedprox_mu is not None and fedprox_mu > 0.0 and global_state_dict is not None:
                    prox_loss = 0.5 * fedprox_mu * fedprox_penalty(model, global_state_dict, DEVICE)
...
def evaluate_loader(model, loader, criterion, class_names, title_prefix="test", save_dir=OUTPUT_DIR, save_cm=True):
    ...
    precision_macro = precision_score(targets, preds, average="macro", zero_division=0, labels=labels)
    recall_macro = recall_score(targets, preds, average="macro", zero_division=0, labels=labels)
    f1_macro = f1_score(targets, preds, average="macro", zero_division=0, labels=labels)
...
def main():
    ...
    for r in range(COMM_ROUNDS):
        ...
        for client_name in CLIENT_NAMES:
            ...
            client_class_weights = compute_class_weights_from_dataset(client_train_ds, num_classes).to(DEVICE)
        ...
        # -------- Global validation --------
        combined_train_weights = compute_class_weights_from_dataset(train_ds_all, num_classes).to(DEVICE)
        combined_criterion = nn.CrossEntropyLoss(weight=combined_train_weights)
