...
def average_representation_state_dict(models, weights):
    """
    Average only keys that belong to the shared representation.
    Head parameters remain unchanged and local.
    """
...
def get_head_state(model):
    sd = model.state_dict()
    return {k: v.detach().cpu().clone() for k, v in sd.items() if k.startswith("head.")}
...
def evaluate_fedrep_across_clients(client_models, test_loaders, class_names, criterion):
    """
    Evaluate a set of personalized client models across their own test sets,
    then aggregate predictions for overall metrics.
    """
    all_targets = []
    all_preds = []
    total_loss = 0.0
    ...
    for client_name in CLIENT_NAMES:
        ...
        loader = test_loaders[client_name]
...
def main():
    ...
    global_val_loader = DataLoader(
        val_ds_all, batch_size=BATCH_SIZE, shuffle=False,
        num_workers=NUM_WORKERS, pin_memory=PIN_MEMORY
    )
    global_test_loader = DataLoader(
        test_ds_all, batch_size=BATCH_SIZE, shuffle=False,
        num_workers=NUM_WORKERS, pin_memory=PIN_MEMORY
    )
    ...
    # Personalized heads: one head per client
    client_head_states = {
    ...
    # Combined class weights for global eval loss
    combined_targets = []
    ...
    for r in range(COMM_ROUNDS):
        ...
        for i, client_name in enumerate(CLIENT_NAMES):
            ...
            # restore this client's personalized head
        ...
        # -------- FedRep aggregation: average only representation --------
        print("\nAggregating shared representation (FedRep)...")
        local_model_list = [local_models[c].cpu() for c in CLIENT_NAMES]
        avg_state = average_representation_state_dict(local_model_list, norm_weights)
        ...
        # Broadcast updated shared representation back to local models
        for client_name in CLIENT_NAMES:
            load_representation(local_models[client_name], avg_state)

        # -------- Global validation using personalized client heads --------
        ...
        print("\nGlobal TEST on combined test (all clients, personalized heads)")
        ...
        per_prec = precision_score(global_test_result["targets"], global_test_result["preds"], average=None, labels=list(range(num_classes)), zero_division=0)
        per_rec = recall_score(global_test_result["targets"], global_test_result["preds"], average=None, labels=list(range(num_classes)), zero_division=0)
        per_f1 = f1_score(global_test_result["targets"], global_test_result["preds"], average=None, labels=list(range(num_classes)), zero_division=0)
