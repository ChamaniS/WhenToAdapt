...
def comogan_harmonize_image(img: Image.Image, client_name: str, split: str, class_name: str, filename: str) -> Image.Image:
    ...
    Everything else in this script is already wired for harmonized outputs.
    """
    img = img.convert("RGB")
...
# Comparison grid
# =========================================================
def make_comparison_grid(original_val_dir: str, harmonized_val_dir: str, client_name: str, out_base: str, n_samples: int = 7):
    grid_dir = ensure_dir(os.path.join(out_base, "ComparisonGrid"))
    orig_imgs = list_images_recursive(original_val_dir)
    harm_imgs = list_images_recursive(harmonized_val_dir)
    if len(orig_imgs) == 0 or len(harm_imgs) == 0:
        print(f"[VIS] no images found for {client_name} comparison grid")
        return
    ...
    for p in harm_imgs:
        b = os.path.basename(p)
        if b not in harm_map:
            harm_map[b] = p

    common = sorted(set(orig_map.keys()) & set(harm_map.keys()))
    pairs = []
    if len(common) > 0:
        for fn in common[:n_samples]:
            pairs.append((orig_map[fn], harm_map[fn], fn))
    else:
        n = min(len(orig_imgs), len(harm_imgs), n_samples)
        for i in range(n):
            pairs.append((orig_imgs[i], harm_imgs[i], os.path.basename(orig_imgs[i])))

    if len(pairs) == 0:
        print(f"[VIS] no matched image pairs for {client_name}")
        return

    top_imgs, mid_imgs, diff_imgs, titles = [], [], [], []
    for orig_p, harm_p, fn in pairs:
        try:
            orig = np.array(Image.open(orig_p).convert("RGB").resize((IMG_SIZE, IMG_SIZE)))
            harm = np.array(Image.open(harm_p).convert("RGB").resize((IMG_SIZE, IMG_SIZE)))
            top_imgs.append(orig)
            mid_imgs.append(harm)
            diff = np.abs(harm.astype(np.float32) - orig.astype(np.float32)) * 3.0
            diff = np.clip(diff, 0, 255).astype(np.uint8)
            diff_imgs.append(diff)
            titles.append(fn)
        except Exception as e:
            print(f"[VIS] skipping pair {fn}: {e}")

    n = len(top_imgs)
    if n == 0:
        return

    fig, axs = plt.subplots(3, n, figsize=(2.6 * n, 7.5))
    if n == 1:
    ...
    for i in range(n):
        ...
        axs[0, i].set_title(titles[i][:24], fontsize=9)
    ...
    fig.suptitle(f"CoMoGAN-style Harmonization: {client_name}", fontsize=14)
...
def build_client_datasets(paths_dict, split, transform):
    ...
    for client in CLIENT_NAMES:
        ...
        ds_list.append(datasets.ImageFolder(split_dir, transform=transform))
...
def main():
    ...
    if HARMONIZE_ALL_SPLITS:
        ...
        for client in CLIENT_NAMES:
            ...
            if SAVE_COMPARISON_GRIDS:
                make_comparison_grid(paths[client]["val"], harmonized_paths[client]["val"], client, OUTPUT_DIR, n_samples=7)
    ...
    train_datasets = [datasets.ImageFolder(train_root_map[c], transform=train_tfms) for c in CLIENT_NAMES]
    val_datasets = [datasets.ImageFolder(val_root_map[c], transform=eval_tfms) for c in CLIENT_NAMES]
    test_datasets = [datasets.ImageFolder(test_root_map[c], transform=eval_tfms) for c in CLIENT_NAMES]
