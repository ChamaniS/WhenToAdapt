...
OUTPUT_DIR = "brain_tumor_federated_fedbn"
MODEL_NAME = "efficientnet_b0_brain_tumor_fedbn.pth"
...
def get_batchnorm_state_keys(model):
    """
    Collect all BatchNorm-related state_dict keys so they can remain client-local.
    """
    bn_keys = set()
...
def extract_bn_state(model, bn_keys):
    """
    Return only BN-related tensors from a model state_dict.
    """
    sd = model.state_dict()
...
def inject_bn_state(model, bn_state):
    """
    Replace the BN-related entries in model.state_dict() with bn_state.
    """
    sd = model.state_dict()
...
def clone_model_with_bn(global_model, bn_state):
    """
    Clone the global model and inject the given client BN state.
    """
...
def save_per_round_checkpoint(global_model, client_bn_states, round_idx, round_metrics, global_val_result, global_test_result, class_names):
    ckpt = {
        "round": round_idx,
        "model_state": global_model.state_dict(),
        "client_bn_states": client_bn_states,
        "global_val_metrics": global_val_result,
        "global_test_metrics": global_test_result,
...
def main():
    ...
    for r in range(COMM_ROUNDS):
        ...
        for client_name in CLIENT_NAMES:
            ...
            # Start from global weights, but restore this client's BN state
        ...
        save_per_round_checkpoint(global_model, client_bn_states, r + 1, round_metrics, global_val_loss, global_test_result, class_names)
