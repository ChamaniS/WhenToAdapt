...
NUM_CLIENTS = 4
LOCAL_EPOCHS = 12        # number of local passes per comm round (tune)
COMM_ROUNDS = 10
...
# Prediction saving
MAX_SAVE_PER_CLIENT = 100   # max number of test images saved per client per round (files overwritten each round)
...
# -------------------------
# Client dataset directories (unchanged)
# -------------------------
train_img_dirs = [
...
# -------------------------
# pFedMe local training function
# -------------------------
def train_local_pfedme(loader, model, loss_fn, inner_steps=INNER_STEPS, inner_lr=INNER_LR,
    ...
    for epoch in range(LOCAL_EPOCHS):
        for data, target in tqdm(loader, leave=False):
            ...
            # inner loop: approximate theta_tilde
...
# -------------------------
# Save predictions utility (overwrites previous predictions in PRED_SAVE_DIR)
# -------------------------
def save_test_predictions(model, test_loader, save_dir, client_id, max_save=MAX_SAVE_PER_CLIENT, device=DEVICE):
    ...
    with torch.no_grad():
        for idx, (img, mask) in enumerate(test_loader):
            ...
            # img: assume normalized with mean 0 std 1 (so values in original range). If you used other normalize, adjust denorm accordingly.
            img_np = img.detach().cpu().numpy()  # B x C x H x W
            mask_np = mask.detach().cpu().numpy()  # B x H x W or B x 1 x H x W?
            pred_np = pred_bin.detach().cpu().numpy()

            # handle shapes and save each sample in the batch
            batch_size = img_np.shape[0]
            for b in range(batch_size):
                if saved >= max_save:
                    break

                # image -> H x W x C, convert to uint8
                im = img_np[b]
                if im.ndim == 3:
                    im = np.transpose(im, (1,2,0))
                # clamp and convert
                im = np.clip(im, 0.0, 1.0)  # if images are in [0,1]
                im_uint8 = (im * 255.).astype(np.uint8)

                # gt mask -> ensure H x W
                m = mask_np[b]
                if m.ndim == 3 and m.shape[0] == 1:
                    m = m[0]
                m_uint8 = (np.clip(m, 0.0, 1.0) * 255.).astype(np.uint8)

                # pred mask -> ensure H x W
                p = pred_np[b]
...
# -------------------------
# Main pFedMe + FedAvg style server (with beta mixing)
# -------------------------
def main():
    tr_tf = A.Compose([
        A.Resize(224,224),
    ...
    global_model = smp.Unet(
        ...
        in_channels=3,
        classes=1,  # binary segmentation, logits output
    ).to(DEVICE)
    round_metrics = []
    ...
    for r in range(COMM_ROUNDS):
        ...
        # Clear/prepare predictions dir so saved files are overwritten each round
        if os.path.exists(PRED_SAVE_DIR):
            # remove existing files to ensure overwrite semantics
            shutil.rmtree(PRED_SAVE_DIR)
        os.makedirs(PRED_SAVE_DIR, exist_ok=True)

        # For each client: copy global into local model, run local pFedMe training
        ...
        # server aggregation with beta mixing:
        ...
        # Evaluate personalized models and save predictions
        rm = {}
        for i in range(NUM_CLIENTS):
            ...
            # compute personalized theta from local model (use some data for inner steps)
            ...
            # Evaluate theta_tilde on test set
            ...
            # Save predictions for this client (overwrites prior round's files because we clear PRED_SAVE_DIR each round)
            client_save_dir = os.path.join(PRED_SAVE_DIR, f"client{i}")
            # ensure per-client dir exists; we clear top-level directory already so subfolders are new
            save_test_predictions(theta_tilde, test_loader, client_save_dir, client_id=i, max_save=MAX_SAVE_PER_CLIENT)
            # saved count printed inside save_test_predictions
