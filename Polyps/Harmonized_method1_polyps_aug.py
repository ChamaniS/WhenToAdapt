...
# Utils
# -------------------------
def get_loader(img_dir, mask_dir, transform, batch_size=1, shuffle=True, return_filename=True):
    """
...
def main():
    ...
    for r in range(COMM_ROUNDS):
        ...
        for i in range(NUM_CLIENTS):
            ...
            train_loader = get_loader(train_img_dirs[i], train_mask_dirs[i], tr_tf, batch_size=1, shuffle=True, return_filename=True)
            val_loader = get_loader(val_img_dirs[i], val_mask_dirs[i], val_tf, batch_size=1, shuffle=False, return_filename=True)
        ...
        rm = {}
        for i in range(NUM_CLIENTS):
            test_loader = get_loader(test_img_dirs[i], test_mask_dirs[i], val_tf, batch_size=1, shuffle=False, return_filename=True)
