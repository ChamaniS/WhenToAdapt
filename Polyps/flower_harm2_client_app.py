...
class FlowerClient(NumPyClient):
    ...
    def evaluate(self, parameters, config):
        ...
        # Save latest predictions for this client (overwrites previous round's latest/)
        # Use a short client label (no spaces) so filenames are tidy.
        client_label = f"Client{self.partition_id}_{_client_name(self.partition_id)}"

        # If Flower/your orchestration provides a round number in config, use it; else None.
        round_num = config.get("round", None) if isinstance(config, dict) else None

        # Save predictions using the helper that preserves original filenames when available.
        # Pass device explicitly and increase max_to_save if you want more masks saved.
        save_test_predictions(
            self.net,
            self.testloader,
            client_label,
...
def client_fn(context: Context):
    ...
    # Read HM flags from pyproject.toml run_config
    ...
    # Load data (will create/reuse histogram-matched copies if enabled)
