...
# -------------------------
# Client dataset directories (set yours)
# -------------------------
train_img_dirs = [
...
# helper to get positive fraction quickly
...
# -------------------------
# Main FedADAM loop (safe) with prediction saving
# -------------------------
def main():
    # transforms
    tr_tf = A.Compose([
    ...
    for r in range(COMM_ROUNDS):
        ...
        # load updated global into global_model
        global_model.load_state_dict(global_sd_cpu, strict=False)

        # Evaluate per-client local models (they still contain their local training)
        rm = {}
        for i, lm in enumerate(local_models):
            test_loader = get_loader(test_img_dirs[i], test_mask_dirs[i], val_tf, shuffle=False)
            ...
            # Save predictions for this client's local model (overwrites that client's latest folder)
            if SAVE_PREDICTIONS:
                save_test_predictions(lm, test_loader, client_names[i], out_dir, round_num=r + 1, device_arg=DEVICE)

        # Diagnostics: global model positive fraction on one example test loader (use first client)
        ...
        # Now overwrite local models so next round they start from global
        for lm in local_models:
