...
def get_weights(net):
    ...
    for name, val in state_dict.items():
        if "encoder" in name or "bottleneck" in name:
            global_weights.append(val.detach().cpu().numpy())
    return global_weights

def set_weights(net, parameters):
    state_dict = net.state_dict()
    keys = [k for k in state_dict.keys() if "encoder" in k or "bottleneck" in k]
    new_state_dict = {}
...
def main():
    ...
    for r in range(COMM_ROUNDS):
        ...
        local_params = [get_weights(lm) for lm in local_models]

        avg_params = []
        for params in zip(*local_params):  # zip across layers
            avg_params.append(sum(p * w for p, w in zip(params, norm_weights)))

        # Update global encoder+bottleneck
        set_weights(global_model, avg_params)

        # Sync global encoder+bottleneck back to clients
        for lm in local_models:
            set_weights(lm, avg_params)

        # ----- Testing with personalized models -----
        rm = {}
        for i in range(NUM_CLIENTS):
