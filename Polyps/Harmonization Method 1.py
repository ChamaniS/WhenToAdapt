#Harmonization method 1: basic preprocessing + normalization + consistent augmentation.
import os
...
# Utils
# -------------------------
def get_loader(img_dir, mask_dir, transform, batch_size=1, shuffle=True):
...
def plot_metrics(round_metrics, out_dir):
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(out_dir, "dice_no_bg_duckmanual.png"))
    plt.close()
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(out_dir, "iou_no_bg_duckmanual.png"))
    plt.close()
...
def main():
    ...
    # - Use a shared augmentation pipeline for all clients' training data
    # - Use a deterministic / minimal pipeline for validation and test
    # - Normalize with ImageNet mean/std (commonly used; replace with dataset mean/std if available)
    # -------------------------
    tr_tf = A.Compose([
