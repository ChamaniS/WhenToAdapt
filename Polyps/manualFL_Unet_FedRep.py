...
# FedRep helpers
# -------------------------
def fedrep_average(models, weights):
    """Average only encoder (representation) parameters across clients (FedRep)."""
    avg_state = copy.deepcopy(models[0].state_dict())
    for key in avg_state.keys():
        if "encoder" not in key and "bottleneck" not in key:  # skip decoders (personalized heads)
            continue
...
def main():
    ...
    for r in range(COMM_ROUNDS):
        ...
        avg_state = fedrep_average(local_models, norm_weights)
        global_model.load_state_dict(avg_state, strict=False)

        # Sync only encoder+bottleneck (shared rep) back to clients
