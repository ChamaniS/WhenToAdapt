...
# -------------------------
# NEW: Input-level MixStyle helper (lightweight, non-invasive)
# -------------------------
def input_mixstyle(images, p=0.6, alpha=0.2, eps=1e-6):
    ...
    x = images.view(B, C, -1)  # B x C x (H*W)
    mu = x.mean(dim=2).view(B, C, 1, 1)      # B x C x 1 x 1
...
def train_local(loader, model, loss_fn, opt):
    ...
    for _ in range(LOCAL_EPOCHS):
        for batch in tqdm(loader, leave=False):
            ...
            # ----- APPLY INPUT-MIXSTYLE HERE (only during training) -----
            # You can tweak p and alpha below.
            try:
                data = input_mixstyle(data, p=0.6, alpha=0.2)
            except Exception:
                # If something goes wrong, fall back to original data (do not break training)
                pass
            # ------------------------------------------------------------
