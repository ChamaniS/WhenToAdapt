...
# NEW: local fine-tuning hyperparams (added)
FINETUNE_EPOCHS = 3        # number of epochs to fine-tune locally
FINETUNE_LR = 1e-5         # smaller LR for fine-tuning
FINETUNE_BATCH = 8       # batch size for fine-tuning (no oversampling)
...
# -------------------------
# Save utilities & save_test_predictions (NEW)
# -------------------------
...
def save_test_predictions(model, test_loader, client_name, out_base, round_num=None, mode="before", device_arg=None):
    ...
    # Clear latest dir then recreate (keeps only the latest saved predictions)
...
# -------------------------
# NEW: Fine-tuning helper (lightweight local personalization)
# -------------------------
...
def main():
    ...
    for r in range(COMM_ROUNDS):
        ...
        # ----- Testing BEFORE fine-tuning (original behavior) -----
        rm = {}
        for i in range(NUM_CLIENTS):
        ...
        # Evaluate fine-tuned models on test sets and store metrics and save predictions
        for i in range(NUM_CLIENTS):
            test_loader = get_loader(test_img_dirs[i], test_mask_dirs[i], val_tf, shuffle=False)
            ...
            # SAVE predictions AFTER fine-tuning
