...
COMM_ROUNDS = 10
CYCLEGAN_EPOCHS = 30   # cycleGAN training epochs (reduce for faster runs)
BATCH_CYCLEGAN = 4
out_dir = "Outputs"
...
# -------------------------
# Simple image-only dataset for CycleGAN training (unpaired)
# -------------------------
...
# -------------------------
# CycleGAN modules (ResnetGenerator & PatchGAN discriminator)
# Minimal implementations adapted for clarity.
# -------------------------
...
class ImagePool:
    # Replay buffer for discriminator (from CycleGAN)
...
# -------------------------
# Train CycleGAN mapping between domainA and domainB
# -------------------------
def train_cyclegan(domainA_dir, domainB_dir, save_dir, epochs=CYCLEGAN_EPOCHS, device=DEVICE):
    ...
    return G_A2B.cpu(), G_B2A.cpu()   # return CPU models for later application

# -------------------------
# Apply generator A->B to all images in src_dir and save to dst_dir (masks copied)
# -------------------------
...
# -------------------------
# Your existing preprocessing / transforms (keep same as earlier)
# -------------------------
tr_tf = A.Compose([
...
# -------------------------
# Main: Train GANs per client (A!=reference) -> harmonize -> FedAvg training
# -------------------------
def main():
    # 1) Train CycleGANs for each non-reference client mapping client -> reference
    cyclegan_models = {}  # store generators for reuse
    for i in range(NUM_CLIENTS):
        ...
        if os.path.exists(final_ckpt):
            # load models (only generator weights)
...
# get_loader wrapper to request filenames if CVCDataset supports return_filename
