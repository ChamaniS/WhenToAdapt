...
# -------------------------
def compute_reference_image_first_sample(ref_img_dir, resize_to=(224,224)):
    """
    Select the FIRST image (sorted) from ref_img_dir as the reference.
    Returns uint8 HxWx3 numpy array.
    """
    ...
    first = files[0]
    p = os.path.join(ref_img_dir, first)
    img = Image.open(p).convert("RGB")
    if resize_to is not None:
        img = img.resize(resize_to, resample=Image.BILINEAR)
    arr = np.array(img).astype(np.uint8)
    return arr, first  # return filename too for logging if needed
...
def create_histogram_matched_datasets(train_img_dirs, train_mask_dirs,
    ...
    Uses the FIRST training sample of the chosen reference client.
    Returns the lists of new directories to use.
    """
    base = os.path.join(out_base, "HistMatch")
    ensure_dir(base)

    # compute reference image from chosen client's train images (first sample) and overwrite cache
    ref_dir = train_img_dirs[reference_client_idx]
    print(f"[HistMatch] Selecting reference image (first sample) from {ref_dir} ...")
    ref_img, ref_name = compute_reference_image_first_sample(ref_dir, resize_to=resize_ref)

    # save reference image for inspection (overwrite any previous)
    ref_save = os.path.join(base, "reference_image.png")
    Image.fromarray(ref_img).save(ref_save)
    print(f"[HistMatch] Reference image (first sample: {ref_name}) saved to {ref_save}")
...
def make_comparison_grid_and_histograms_updated(img_dir, mask_dir, val_transform, visual_transform,
    ...
    Comparison grid:
     ...
     - Middle row: HISTOGRAM-MATCHED images (matched to FIRST Kvasir training sample)
     - Bottom row: Amplified absolute color difference (orig_resized vs matched)
    This function ALWAYS selects the FIRST training sample of the Kvasir dataset (reference_client_idx)
    as the histogram-match reference and overwrites the cached reference image.
    """
    ...
    # Always select the FIRST training image from reference_client_idx (Kvasir) as reference (resized to target)
    reference_img = None
    try:
        ref_dir = train_img_dirs[reference_client_idx]
        ...
        first_rf = ref_files[0]
        p = os.path.join(ref_dir, first_rf)
        im = Image.open(p).convert("RGB").resize((target_w, target_h), resample=Image.BILINEAR)
        reference_img = np.array(im).astype(np.uint8)
        # save/overwrite reference for later (so cache won't contain an old average)
        ...
        print(f"[make_comparison_grid] Selected and saved reference image (first sample) from {ref_dir}: {first_rf}")
    except Exception as e:
        print(f"[make_comparison_grid] Failed to select reference image: {e}. Mid-row will fallback to val_transform per-image.")
        reference_img = None
    ...
    for fname in fnames:
        ...
        if reference_img is not None:
            try:
                ...
                matched = np.clip(matched, 0, 255).astype(np.uint8)
                # resize matched to target size for display consistency (already same size, but ensure)
    ...
    fig.suptitle(f"Harmonized (Histogram matching using FIRST Kvasir sample) vs. Original: {client_name}", fontsize=16, y=0.98)
    fig.text(0.035, 0.82, "Original\nimages", fontsize=12, va='center', rotation='vertical')
    fig.text(0.035, 0.50, "Histogram-\nmatched \n (First sample)", fontsize=12, va='center', rotation='vertical')
...
def plot_metrics(round_metrics, out_dir):
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(out_dir, "dice_no_bg_harm2_m2.png"))
    plt.close()
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(out_dir, "iou_no_bg_harm2_m2.png"))
    plt.close()
